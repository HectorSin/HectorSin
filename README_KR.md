<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:667eea,100:764ba2&height=220&text=AI%20Engineer&animation=fadeIn&fontColor=ffffff&fontSize=70&fontAlignY=38&desc=LLM%20기반%20실무%20서비스%20개발자&descAlignY=62&descSize=18" />
</div>

<div align="center">
  
  [![English](https://img.shields.io/badge/Language-English-blue?style=for-the-badge)](README.md)
  [![한국어](https://img.shields.io/badge/언어-한국어-red?style=for-the-badge)](README_KR.md)
  
</div>

<div align="center">
  <h1>👋 안녕하세요, 신재현입니다!</h1>
  <h3>AI 서비스 엔지니어 | LLM 기반 시스템 개발</h3>
  
  <!-- <br>
  <br>
  
  [![Hits](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fgithub.com%2FHectorSin&count_bg=%23667EEA&title_bg=%23764BA2&icon=github.svg&icon_color=%23FFFFFF&title=방문자+수&edge_flat=false)](https://hits.seeyoufarm.com) -->
  
</div>

<br>

## 🎯 소개

| | |
|---|---|
| **직무** | AI Engineer |
| **전문분야** | LLM 기반 서비스 개발 및 자동화 |
| **학력** | 아주대학교 (AI융합학부 + e비즈니스학과 복수전공) |
| **언어** | 한국어 (모국어), 영어 (유창), 스페인어 (유창) |
| **구직 중** | AI/ML 엔지니어, LLM 서비스 엔지니어, 데이터 엔지니어 |

### 💡 주요 성과

**FieldU** - LLM 자동화로 콘텐츠 제작 비용 100% 절감

**TurbinCrew** - IoT → AWS → ML 데이터 파이프라인 구축

**SnackCast** - AI 뉴스 팟캐스트 자동 생성 서비스

> **모토**: *"AI 모델을 측정 가능한 비즈니스 성과로 전환"*
  
</div>

<br>

## 🤖 LLM & AI 서비스 (핵심 역량)

<div align="center">

### 🎨 LLM API & 모델

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Google](https://img.shields.io/badge/Gemini_API-4285F4?style=for-the-badge&logo=google&logoColor=white)
![Anthropic](https://img.shields.io/badge/Claude_API-181818?style=for-the-badge&logo=anthropic&logoColor=white)

</div>

**사용 경험:**

**Gemini API** - FieldU 콘텐츠 생성 파이프라인의 핵심 기술

**Perplexity API** - SnackCast 뉴스 수집 시스템

**OpenAI GPT-4** - 프로젝트 프로토타이핑 및 실험

**Claude API** - 고품질 텍스트 생성 및 분석

**Naver Clova TTS** - SnackCast 팟캐스트 음성 합성 통합

<div align="center">

### 🔗 AI 프레임워크 & 기술

![LangChain](https://img.shields.io/badge/🦜_LangChain-1C3C3C?style=for-the-badge)
![RAG](https://img.shields.io/badge/🔍_RAG-4CAF50?style=for-the-badge)
![Prompt](https://img.shields.io/badge/✍️_Prompt_Engineering-FF6B6B?style=for-the-badge)

</div>

**💡 핵심 기술:**

**Multi-LLM 통합 및 오케스트레이션** - 여러 LLM API를 조합한 통합 워크플로우 구축

**콘텐츠 생성 파이프라인** - 품질 관리가 통합된 대규모 자동화 시스템 개발

**RAG (Retrieval-Augmented Generation)** - 벡터 데이터베이스 기반 검색 증강 생성

**비용 최적화 및 토큰 관리** - 프로덕션 환경의 효율성 및 모니터링

**프로덕션 API 설계** - 안정적이고 확장 가능한 서비스 아키텍처

<br>

## 💻 백엔드 & 인프라

<div align="center">

### 🌐 백엔드 개발

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)

### ☁️ 클라우드 & DevOps

![GCP](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)

</div>

**클라우드 경험:**

**GCP**: SnackCast 프로덕션 배포를 위한 Cloud Run 및 Cloud Storage 활용

**AWS**: TurbinCrew 실시간 데이터 파이프라인을 위한 IoT Core 및 Lambda 구축

<div align="center">

### 🗄️ 데이터베이스

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

</div>

**Vector DB:** Chroma, Pinecone (RAG 시스템용)

<div align="center">

### 🛠️ 개발 도구

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Notion](https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white)
![Slack](https://img.shields.io/badge/Slack-4A154B?style=for-the-badge&logo=slack&logoColor=white)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)

### 🔬 머신러닝 & 딥러닝

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)

</div>

<br>

## 💼 실무 경험 & 프로젝트

<div align="center">

### 🚀 임팩트 중심 프로젝트

| 프로젝트 | 역할 | 주요 성과 | 기술 스택 |
|---------|------|-----------|-----------|
| **FieldU** 📚 | AI 서비스 인턴 | **콘텐츠 제작 비용 100% 절감** | Gemini API, LangChain, Python |
| **TurbinCrew** ⚡ | AI 서비스 인턴 | **IoT → 클라우드 → ML 파이프라인 구축** | AWS IoT Core, Lambda, Python |
| **SnackCast** 📻 | 리드 개발자 | **AI 뉴스-팟캐스트 자동화 서비스** | FastAPI, GCP, Multi-LLM APIs |

</div>

<details>
<summary><b>📚 FieldU - 교육 AI 스타트업 (인턴)</b></summary>

<br>

**🎯 과제:** 수작업 콘텐츠 제작이 시간과 비용 소모

**💡 솔루션:** LLM 기반 콘텐츠 자동 생성 파이프라인 구축

**📊 성과:**

콘텐츠 제작 워크플로우 완전 자동화를 통한 **비용 100% 절감** 달성

최적화된 AI 파이프라인으로 콘텐츠 제작 프로세스 **시간 50% 단축** 실현

통합 품질 관리 메커니즘을 갖춘 확장 가능한 다단계 AI 워크플로우 구축

**🔧 기술 하이라이트:**

도메인 특화 콘텐츠를 위한 커스텀 프롬프팅 전략과 Gemini API 통합 구현

정교한 다단계 콘텐츠 정제 및 검증을 위한 LangChain 활용

일관된 출력 품질을 보장하는 자동화된 품질 보증 파이프라인 설계

</details>

<details>
<summary><b>⚡ TurbinCrew - 에너지 테크 스타트업 (인턴)</b></summary>

<br>

**🎯 과제:** IoT 센서 데이터로 풍력 발전량 실시간 예측

**💡 솔루션:** 센서부터 ML 모델까지 End-to-End 데이터 파이프라인

**📊 성과:**

엔드투엔드 데이터 흐름을 위한 **AWS IoT Core → Lambda → ML 모델** 통합 성공

풍력 발전량 예측을 위한 실시간 데이터 처리 및 예측 시스템 배포

베트남 ELECS 2024 전시회에서 기술 전문성을 기반으로 **3개 MOU 체결**에 기여

**🔧 기술 하이라이트:**

확장 가능한 ML 배포를 위한 IoT 센서 데이터 수집 및 전처리 인프라 설계

대규모 데이터 처리를 위한 AWS 클라우드 인프라 설계 및 구현

동남아시아 파트너사 대상 국제 비즈니스 기술 발표 수행

</details>

<details>
<summary><b>📻 SnackCast - AI 뉴스 팟캐스트 (캡스톤 프로젝트)</b></summary>

<br>

**🎯 과제:** 뉴스 큐레이션 → 대본 생성 → 팟캐스트 제작 전 과정 자동화

**💡 솔루션:** Multi-API 오케스트레이션 파이프라인

**📊 성과:**

Google Cloud Platform 인프라 상에서 **프로덕션 배포** 완료

뉴스 수집 → 대본 생성 → 음성 합성 엔드투엔드 자동화 파이프라인 구현

텍스트와 음성 기술을 결합한 멀티모달 AI 컴포넌트 통합 성공

**🔧 기술 하이라이트:**

지능적인 뉴스 수집 및 큐레이션을 위한 Perplexity API 활용

맥락 인식 팟캐스트 대본 생성을 위한 Gemini API 레버리징

자연스러운 한국어 음성 합성을 위한 Naver Clova TTS 통합

확장 가능한 서비스 제공을 위해 GCP Cloud Run에 배포된 FastAPI 백엔드 구축

**🔗 링크:** [프로젝트 상세 보기](#) *(추후 추가)*

</details>

<br>

## 🏆 주요 활동 & 성과

<div align="center">

| 분야 | 내용 |
|------|------|
| 🎯 **알고리즘** | 백준 온라인 저지 활동 |
| 🤖 **ML 대회** | Open Ko-LLM Leaderboard Season 3 참여 |
| 📝 **기술 콘텐츠** | LinkedIn AI 트렌드 포스팅<br>(AI Agents, RAG, LLM 최신 동향) |
| 👥 **리더십** | Friends 문화교류 동아리 회장<br>COVID-19 기간 중 **4명 → 130명** 성장 |
| 🌍 **글로벌 경험** | 멕시코 (2002-2004), 아르헨티나 (2011-2014) 거주 |
| 🎤 **크리에이티브** | 성우 및 더빙 경험 |

</div>

<br>

## 📊 GitHub 통계

<div align="center">
  
  <!-- <img src="https://github-readme-stats.vercel.app/api?username=HectorSin&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&title_color=667eea&icon_color=764ba2&text_color=ffffff" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=HectorSin&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=667eea&text_color=ffffff&langs_count=6" width="48%" />
  
  <br><br> -->
  
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=HectorSin&theme=radical&hide_border=true&background=0D1117&stroke=667eea&ring=764ba2&fire=764ba2&currStreakLabel=764ba2" width="48%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=HectorSin&theme=radical" width="98%" />
  
</div>

<br>

## 📫 연락처

<div align="center">
  
  [![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:jaehyunkkang15634@gmail.com)
  [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/YOUR_LINKEDIN_ID)
  [![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=notion&logoColor=white)](YOUR_NOTION_URL)
  
  <br>
  
  ### 🌟 현재 구직 중
  
  ```
  🔍 AI/ML 엔지니어
  🔍 LLM 서비스 엔지니어
  🔍 데이터 엔지니어
  ```
  
  ### 💭 현재 관심사
  
</div>

🔭 측정 가능한 ROI와 비즈니스 임팩트를 제공하는 프로덕션 LLM 애플리케이션 개발

🌱 복잡한 워크플로우를 위한 고급 RAG 기술 및 멀티 에이전트 시스템 탐구

💬 기술 콘텐츠 및 지식 공유를 통한 한국 AI 커뮤니티 기여

🎯 최첨단 AI 연구와 실제 비즈니스 가치 사이의 가교 역할 수행

<br>

## 🎓 학력

<div align="center">

**아주대학교** (2026년 2월 졸업)

**AI융합학부** (주전공) | **e비즈니스학과** (복수전공)

</div>

<br>

---

<div align="center">
  
  ### 📌 재미있는 사실
  
  > "3개국(한국 🇰🇷, 멕시코 🇲🇽, 아르헨티나 🇦🇷)에서 성장하며  
  > 3개 언어를 유창하게 구사하고,  
  > 이제는 기계에게도 말하는 법을 가르칩니다! 🤖"
  
  <br>
  
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:667eea,100:764ba2&height=120&section=footer" />
  
</div>
